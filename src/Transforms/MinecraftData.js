import * as nbt from "./NBT.class.js";
import { inflate, deflate } from "https://deno.land/x/zlib.es@v1.0.0/mod.ts";
import PartialReadError from "https://raw.githubusercontent.com/NowhereLTD/deno-protodef/master/src/utils.js";

const encoder = new TextEncoder();
const decoder = new TextDecoder();

const UUID = [readUUID, writeUUID, 16];
const nbt = [readNbt, writeNbt, sizeOfNbt];
const optionalNbt = [readOptionalNbt, writeOptionalNbt, sizeOfOptionalNbt];
const compressedNbt = [readCompressedNbt, writeCompressedNbt, sizeOfCompressedNbt];
const restBuffer [readRestBuffer, writeRestBuffer, sizeOfRestBuffer];
const entityMetadataLoop = [readEntityMetadata, writeEntityMetadata, sizeOfEntityMetadata];
const topBitSetTerminatedArray = [readTopBitSetTerminatedArray, writeTopBitSetTerminatedArray, sizeOfTopBitSetTerminatedArray];
...
function readUUID (buffer, offset) {
  ...
  return {
    value: decoder.decode(buffer.slice(offset, 16 + offset)),
    size: 16
  }
}
...
function readCompressedNbt (buffer, offset) {
  ...
  const nbtBuffer = deflate(compressedNbt) // TODO: async
...
function writeCompressedNbt (value, buffer, offset) {
  ...
  const compressedNbt = inflate(nbtBuffer) // TODO: async
